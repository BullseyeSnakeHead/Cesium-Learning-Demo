...
const rectPrimitive = new Cesium.Primitive({
  ...
  // apperence可以改变实例的材质  默认属性用PerInstanceColorAppearance，配合instance的attributes使用。自定义材质用MateriApperance见main4
...
btn.addEventListener('click',() => {
  // 修改图片红色矩形的颜色 颜色的设置是在instance中,instance需要id配置,获取instance用下面这个很长的方法，对象是图元，调用图元的instance方法，找id
  const attr = rectPrimitive.getGeometryInstanceAttributes('rect')
  // 修改颜色，注意哈，颜色的修改是在instance的arrtibute实现的，所以这里需要调用ColorGeometryInstanceAttribute找到attributes修改tovalue
  attr.color = Cesium.ColorGeometryInstanceAttribute.toValue(Cesium.Color.YELLOW.withAlpha(0.5))
})

// 城市白模


// 交互效果ScreenSpaceEventHandler,对象是整个三维场景vierer.scene.cavans
const handler = new Cesium.ScreenSpaceEventHandler(viewer.scene.canvas)
// cesium的事件监听  1.回调函数,在此步可以添加位置判断 2.激活方法cesium专用的click方法 
handler.setInputAction(
  // 视口偏移position,pick方法限定在图元范围内,在图元范围外点击不会打印,有点像filter筛选
  (movement) =>{
    const pickedObject = viewer.scene.pick(movement.position)
    // 可以打印对应图元的id
    console.log(pickedObject.id);
    // 所以以图元id的存在与否,可以做判断是否在图元内点击 defined判断是否是cesium对象
    if(Cesium.defined(pickedObject) && typeof pickedObject.id === 'string') {
     alert(pickedObject.id)
    }
  },
  // 触发方法
